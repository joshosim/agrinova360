...
import { fetchOrganizationName, getProfilePhoto, pickAndUploadProfilePhoto } from '@/utils/helpers';
  ...
  return (
    <ScrollView
      ...
      <View style={{ alignItems: 'center', justifyContent: 'center', position: 'relative', marginTop: 50 }}>
        <Image key={photoUrl} source={{
          uri: photoUrl || 'data:image/jpeg;base64'
        }}
        ...
        <View style={{
          ...
          <Ionicons
            onPress={async () => {
              try {
                const newUrl = await pickAndUploadProfilePhoto(user?.id!);
                if (newUrl) {
                  // bust cache so it always reloads immediately
